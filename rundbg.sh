...
case "$1" in
	...
	for i in ${!entity_nss[@]}; do
	    $vppctl create host-interface name ${entity_nss[i]}-$nic_name num-rx-queues 2 num-tx-queues 2
	...
	$vppctl set acl-plugin acl $rule1 tag example_permit_1
	...
	$vppctl set acl-plugin interface host-${entity_nss[0]}-$nic_name input acl 2
	$vppctl set acl-plugin interface host-${entity_nss[0]}-$nic_name output acl 0
	$vppctl set acl-plugin interface host-${entity_nss[0]}-$nic_name output acl 2

...
#
# Client 10.10.1.2/24 -- 10.10.1.1/24 CGW 10.100.1.1 -- 10.100.1.101 RGW 10.100.1.103 -- 10.100.1.3 Router
# Server 10.20.1.2/24 -- 10.20.1.1/24 SGW 10.100.1.2 -- 10.100.1.102 RGW 10.100.1.103 -- 10.100.1.3 Router
#
# route rule
...
# SGW
#  - default route to RGW(10.100.1.3) veth0
#  - 10.20.1.0/24 to Server(10.10.1.2) server-veth0
# RGW
...
case "$1" in
    test)
	echo make_vethpair veth0 $nscgw $nsrgw 10.100.1.1/24 10.100.1.101/24 
	make_vethpair veth0 $nscgw $nsrgw 10.100.1.1/24 10.100.1.101/24
	echo "done"
	    ;;
    ns|netns)
	...
	make_vethpair veth0 $nscgw $nsrgw 10.100.1.1/24 10.100.1.101/24 
	make_vethpair veth0 $nssgw $nsrgw 10.100.1.2/24 10.100.1.102/24 
	make_vethpair veth0 $nsr $nsrgw 10.100.1.3/24 10.100.1.103/24 default
	...
	vppctl_with $id set acl-plugin interface host-${entity_nss[1]}-${nic_names[$i]} input acl 0
	...
	vppctl_with $id set acl-plugin interface host-${entity_nss[1]}-${nic_names[$i]} output acl 0
